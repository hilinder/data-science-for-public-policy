...
ggplot() +
     ...
     geom_sf(data = bikelane_intersection, color = "red", alpha = .5) 
```
```{r creating percentage variables}
sac_centroid_select <- sac_centroid_select %>%
...
```{r gathering for plotting}
view( sac_centroid_select %>%
    st_drop_geometry() %>%
    select(GEOID, total_whiteE, bikelane_density, med_yearlyincome_2021adjustedE, med_ageE, med_housingcostE, total_povertyE, total_bachelors_ages25plusE, pctWhite, pctBachelors, pct25to29, pctPoverty) %>%
    gather( key = "variable", value = "value", -GEOID, -bikelane_density)
    )
```
```{r plotting all my variables against bikelane density}
sac_centroid_select %>%
    st_drop_geometry() %>%
    select(GEOID, total_whiteE, bikelane_density, med_yearlyincome_2021adjustedE, med_ageE, med_housingcostE, total_povertyE, total_bachelors_ages25plusE, pctWhite, pctBachelors, pct25to29, pctPoverty) %>%
    gather( key = "variable", value = "value", -GEOID, -bikelane_density) %>%

ggplot(aes(x = bikelane_density, y = value)) +
    geom_point() +
    geom_smooth(method = "lm", se = F) +
    facet_wrap(~variable, ncol = 3, scales = "free")
```
As I expected, there's a positive correlation between a tracts whiteness, education level, and amount of young people. There's less of a correlation with income. 
